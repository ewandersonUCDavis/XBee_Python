#This script collects DIO data from several remote XBees and sends the data to several thingsboard devices through a thingsboard gateway.
...
while True:
    try:
		...
		if ('samples' in response): #Is this a DIO sample frame?
			...
			print('one')
			print(response)
			print(response.get('source_addr_long'))
			temp = turbineList.keys()
			print(temp)
			print(type(temp[1]))
