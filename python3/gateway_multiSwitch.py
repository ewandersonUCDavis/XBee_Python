...
def on_connect(MQTT, userdata, flags, rc):
    ...
    MQTT.subscribe("v1/gateway/attributes/response") #subscribe to receive answers when we request attributes from the MQTT server (this topic corresponds to attributes of devices connected to the gateway, not gateway attributes)
...
def readTurbineInputFile(): 
	...
	for i in range(1, len(turbArrayProps)): #process each row in input file
		...
		MQTT.publish(topicAttr,json.dumps({name:{'turbineOnOff_switch':True}}), qos=1, retain=True) #Comment this out after the first time you run the program. The first time you need it to establish these attributes.
		#MQTT.publish('v1/gateway/attributes/request', json.dumps({"id": i, "device": name, "client": True, "key": "turbineOnOff_switch"}), qos=1, retain=True) #This line requests the status of this turbine's on/off switch from thingsboard
...
def listener():
	while True:
		...
		if new_MQTT_message:
			...
			#This can be simplified if we aren't requesting attributes from thingsboard
			#if MQTT_message.topic[0:30] == 'v1/gateway/attributes/response': #This message is a response to our switch status attribute request (only happens when we connect to the MQTT server)
			...
			#elif MQTT_message.topic[0:14] == 'v1/gateway/rpc' : #This is an RPC message from one of the switches
...
#MQTT.publish('v1/devices/me/attributes/request/1', '{"clientKeys":"safety_switch,turbineOnOff_switch"}', qos=1, retain=True) #This function has been moved to the input file read.
