#This script switches to the official Digi xbee python libraries. It also implements start/stop sequencing commands to the remote xbees. this script must be run in python 3, not python 2.
...
# 			print(IO_Data)
# 			print(type(IO_Data.get('overspeed_fault')))
# 			if (IO_Data.get('overspeed_fault') or IO_Data.get('vibration_fault') or IO_Data.get('overtemp_fault') or IO_Data.get('low_voltage_fault') or IO_Data.get('other_fault')) : #check to see if this turbine is experiencing any faults
# 				print('Fault detected')
# 				IO_Data['anyFault'] = True #yes, at least one fault
# 			else:
# 				IO_Data['anyFault'] = False #no, no faults
# 			message = {turbineName: IO_Data} 
# 			print(message)
